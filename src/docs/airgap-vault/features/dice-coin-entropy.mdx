...
We've been aware of this issue with random number generators since the first release of AirGap. As a result, before creating a secret, we collect additional entropy from the camera, microphone, accelerometer and touch input to decrease the attack vector from a compromised RNG. The resulting seed phrase will include both the RNG output and the inputs from the users. If someone wants to compromise and recalculate the secret, he must know both inputs, therefore a compromised RNG is no longer sufficient to steal your seed in AirGap.

To take security further and eliminate any flaws from the RNG, we introduced the dice roll and coin flip feature. Instead of relying on the device to give you random numbers to generate the mnemonic, the users have to provide those random numbers themselves using either dice or a coins.

## Advantages of Coin Flip and Dice Roll Entropy Collection

- ### Elimination of RNG failure
  Since the RNG is no longer involved when creating mnemonics, the associated risks are no longer an issue.
...
- In the case of coin flips, 256 flips are required to achieve the randomness or entropy for a 24-word mnemonic.
...
:::note
There are multiple standards of how dice roll inputs can be interpreted. We have the two methods in AirGap. The default method and ColdCard method. The different methods are only relevant if you plan to verify the mnemonic with another wallet.
:::

To verify the generated seed phrase using the cold card method, you can follow this [guide](https://coldcard.com/docs/verifying-dice-roll-math).
...
- Click the understood button to generate your mnemonic from your dice roll input.

Hurray, you've successfully generated your mnemonic using dice rolls!
...
- Click the understood button to generate your mnemonic from your coin flip input.

Hurray, you've successfully generated your mnemonic from using coin flips!.
