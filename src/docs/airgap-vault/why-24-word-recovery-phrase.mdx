...
AirGap Vault does support mnemonics that are shorter than 24 words. Specifically, it supports mnemonics of length 12, 15, 18, 21, and 24. However, when generating a new mnemonic with AirGap Vault, the output will always be a 24-word recovery phrase.
...
We chose to generate 24 words(256-bit) entropy not because 12 word mnemonics(128-bit) entropy is insecure, but the handling of shorter mnemonics is riskier in a few ways.
