...
Yes, but the unlisted ones will not be visible in the AirGap Wallet, but you can connect to any ethereum DApp using the walletconnect functionality to interact with such token.
...
Some protocols (Tezos, Cosmos, Polkadot) allow you to "delegate" your funds and earn rewards. Delegations are slightly different in each protocol in terms of reward and risk. As a result, we recommend you conduct some preliminary research on your desired protocol. Tezos is the easiest, allowing you to delegate your whole tez with no risk, and without locking in your funds while receiving approximately 5 percent in incentives per year.

## Does AirGap allow freezing of UTXO?

No, the AirGap Wallet companion app is a very simple wallet and doesn't offer any BTC specific features.
