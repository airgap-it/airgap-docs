...
## Can I import the btc wallet created in the AirGap vault into other wallets using the private key?

You cannot get the private key in the Vault but you can import the mnemonic phrase into any wallet that is an Hierarchical Deterministic wallet. 

## How long does it take to exchange on AirGap

This may take 20-30 minutes

## If I send some AE, do I need to set fees, or AirGap does it automatically, since the slider rests on disabled?

The fee slider is on usually on "slow & cheap" which is a big enough fee for AE at the moment but fee can also be adjusted after clicking the toggle button on top of the slider

## Is it possible to have a new coin listed on AirGap?

Yes it is possible, we want to support as many chains and coins as possible but we also follow a procedure of getting in touch with the teams for a potential integration.

## What are the available exchange pairs?

The list of available exchange pairs can be seen on the exchange tab. This functionality is provided to us by third parties( changelly and change now) and may vary based on what they provide.

## How do I go about ensuring the AirGap vault stays up to date if not actually connected to the internet, what do we recommend?
...
AirGap does not set any minimum balance requirement on any token or coin. This is a design on the Polkadot network, an address is only active when it holds a minimum amount (currently set at 1 DOT). This minimum amount is called an existential deposit (ED).

## Can I send NFT to my AirGap wallet?

Yes, you can send NFT to AirGap. But you can't see them inside AirGap Wallet, but you will see them on the dApps that support those NFTs.

## How long does it take to unbound in polka dot?

Unbonding takes about 28 days
