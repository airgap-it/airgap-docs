...
## What is AirGap business model/ how does Airgap make money?
...
You can recover your mnemonics if you still have access to the vault by going to settings > select account > show mnemonics. Unfortunately, It also means that your funds cannot be accessed if you do not have access to the vault.

## Why does AirGap Vault generate looping qr code for signing ERC20 token transactions?

ERC 20 transactions can contain more data than a single QR code can display. In this case, it is split up into multiple parts.
